...
class TaoTree(BaseEstimator):
    ...
    def _tao_iter_cart(self, X, y, tree, X_score=None, y_score=None, sample_weight=None):
        ...
        while len(queue) > 0:
            ...
            if self.weights.get(node_id) is None and feature[node_id] != -2:
                self.weights[node_id] = np.zeros(X.shape[1])
                self.weights[node_id][feature[node_id]] = 1
        ...
        for (node_id, path_to_node_index) in reversed(indexes_with_prefix_paths):
            ...
            def filter_points_by_path(X, y, path_to_node_index):
                ...
                for node_id, direction in path_to_node_index:
                    idxs = np.dot(X, self.weights[node_id]) <= threshold[node_id]
                    if direction == 'R':
                        idxs = ~idxs
            ...
            if children_left[node_id] == children_right[node_id]:  # is leaf node
                if isinstance(self, RegressorMixin) and X_node.shape[0] >= self.min_leaf_samples_tao:
                    ...
                    """
                elif isinstance(self, ClassifierMixin) and X_node.shape[0] >= self.min_leaf_samples_tao:
                    freq = np.bincount(y_node.astype(int))
                    while len(freq) < len(value[node_id][0]):
            ...
            def predict_from_node(X, node_id):
                ...
                def predict_from_node(x, node_id):
                    ...
                    if np.dot(x, self.weights[node_id]) <= threshold[node_id]:
            ...
            if len(np.unique(y_node_target)) == 1:
                if y_node_target[0] == 0:
    ...
    def predict(self, X):
        ...
        preds = np.zeros(X.shape[0])
        for i in range(X.shape[0]):
            preds[i] = predict_from_node(X[i], 0)
        return preds
